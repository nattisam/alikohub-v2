          ...
          <Card key={c.id} className={!c.is_read ? "border-primary/30" : ""}>
            <CardContent className="p-4 flex items-center justify-between flex-wrap gap-3">
              <div className="min-w-0">
                ...
                <div className="text-sm text-muted-foreground">{c.email} • {c.service_interest || "General"}</div>
                <div className="text-xs text-muted-foreground">{new Date(c.created_at).toLocaleString()}</div>
              </div>
              <div className="flex gap-2">
                <Button variant="ghost" size="icon" onClick={() => markRead(c)}><Eye className="w-4 h-4" /></Button>
                <Button variant="ghost" size="icon" asChild><a href={`mailto:${c.email}`}><Mail className="w-4 h-4" /></a></Button>
                <Button variant="ghost" size="icon" onClick={() => handleDelete(c.id)}><Trash2 className="w-4 h-4 text-destructive" /></Button>
              </div>
            </CardContent>
          </Card>
        ))}
        {items.length === 0 && <p className="text-muted-foreground text-center py-8">No contact submissions yet.</p>}
      </div>
        ...
        <DialogContent className="max-w-lg">
          ...
          {selected && (
            <div className="space-y-3 text-sm">
              ...
              {selected.organization && <div><strong>Organization:</strong> {selected.organization}</div>}
              {selected.country && <div><strong>Country:</strong> {selected.country}</div>}
              {selected.service_interest && <div><strong>Service:</strong> {selected.service_interest}</div>}
