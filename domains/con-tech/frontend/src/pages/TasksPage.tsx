...
const TasksPage = () => {
  ...
  const { data: allTasks, isLoading: tasksLoading, isError, refetch } = useTasks(
    ...
    return (
      <div className="flex h-screen bg-slate-50">
        ...
        <div className="flex-1 flex flex-col justify-center items-center">
          ...
          <p className="mt-4 text-slate-500 font-medium animate-pulse">Loading tasks...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-auto animate-in fade-in duration-500">
        <header className="bg-white border-b border-slate-100 sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-6 py-6 sm:px-8 lg:px-10 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-2xl font-black text-slate-900 tracking-tight uppercase">Tasks</h1>
              <p className="text-slate-500 text-xs mt-1 font-medium italic">Manage project deliverables</p>
            </div>
            <button
        ...
        <main className="max-w-7xl mx-auto px-6 py-8 sm:px-8 lg:px-10">
          {isError && (
            <div className="mb-8 p-4 bg-red-50 border border-red-100 rounded-3xl flex items-center gap-4 animate-pulse">
              ...
              <div>
                <p className="text-xs font-black text-red-900 uppercase tracking-widest leading-none">Synchronization Offline</p>
                <p className="text-[10px] font-bold text-red-400 mt-1 italic uppercase leading-none">Live data verification failed. Displaying cached scope.</p>
              </div>
            </div>
          )}
          ...
          {filteredTasks.length > 0 ? (
            ...
            <EmptyState 
              title={searchTerm ? "No matching tasks" : (selectedProject === 'all' ? "No tasks found" : "No tasks for this project")}
              message={searchTerm ? "Try adjusting your filters or search term." : "Get started by creating a new task for your project."}
