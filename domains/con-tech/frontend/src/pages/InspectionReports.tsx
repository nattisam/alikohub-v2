...
const InspectionReports: React.FC = () => {
  ...
  const fetchInspections = async () => {
    ...
    try {
      ...
      const inspectionData = await inspectionsService.findAllForProject(selectedProject, {});
  ...
  return (
    <main className="p-6 w-full">
      <div className="mb-6 w-full">
        ...
        <p className="text-gray-600">View quality assessments and inspection reports for your projects</p>
      </div>
          ...
          {inspections.length > 0 ? (
            <div className="grid grid-cols-1 gap-6">
              {inspections.map((inspection) => (
                <div key={inspection.id} className="bg-white p-6 rounded-lg shadow-md">
                  <div className="flex justify-between items-start">
                    <div>
                      <h2 className="text-xl font-semibold">Inspection #{inspection.id}</h2>
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium mt-2 ${getStatusColor(inspection.status)}`}>
                  ...
                  <div className="mt-4">
                    ...
                    <p className="mt-1 text-gray-600 whitespace-pre-wrap">{inspection.findings || 'No findings reported'}</p>
                  </div>
                  
                  <div className="mt-4">
                    <h3 className="font-medium text-gray-900">Recommendations</h3>
                    <p className="mt-1 text-gray-600 whitespace-pre-wrap">{inspection.recommendations || 'No recommendations provided'}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md text-center">
              <h2 className="text-xl font-semibold mb-2">No Inspection Reports Found</h2>
