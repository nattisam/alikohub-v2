  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="mt-4 text-sm font-medium text-gray-500">Retrieving Project Status...</p>
      </div>
    );
  }
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Real-time transparency and progress tracking</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <Card className="lg:col-span-2 border-none shadow-sm">
          <CardHeader className="flex flex-row items-center justify-between border-b border-gray-50">
            <CardTitle className="text-lg font-bold text-gray-900">Latest Project Activity</CardTitle>
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-gray-50 max-h-[400px] overflow-auto">
              {(clientData.recentUpdates || []).length > 0 ? (clientData.recentUpdates || []).map((update) => (
                <div key={update.id} className="p-6 hover:bg-gray-50 transition-colors flex justify-between items-center group">
                  <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-[#3E92D1]">
                      <CheckCircle2 className="w-5 h-5" />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900 group-hover:text-[#3E92D1] transition-colors">{update.title}</p>
                ...
                <div className="p-10 text-center text-gray-400 text-sm italic">No recent activities logged from construction sites.</div>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-gray-900">Quality Control</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-emerald-50 rounded-xl border border-emerald-100">
                  <span className="text-xs font-medium text-emerald-800 uppercase tracking-wider">Passed</span>
                  <span className="text-xl font-bold text-emerald-600">{clientData.inspectionSummary?.passed || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-amber-50 rounded-xl border border-amber-100">
                  <span className="text-xs font-medium text-amber-800 uppercase tracking-wider">Pending</span>
                  <span className="text-xl font-bold text-amber-600">{clientData.inspectionSummary?.pending || 0}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-red-50 rounded-xl border border-red-100">
                  <span className="text-xs font-medium text-red-800 uppercase tracking-wider">Failed</span>
                  <span className="text-xl font-bold text-red-600">{clientData.inspectionSummary?.failed || 0}</span>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="border-none shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-gray-900">Quick Resources</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <div className="pt-4 mt-4 border-t border-gray-100 text-center">
                <p className="text-[10px] text-gray-400 font-medium uppercase tracking-widest">System Status</p>
                <div className="flex items-center justify-center gap-2 mt-1">
                  <div className={`w-2 h-2 rounded-full ${isError ? 'bg-red-500' : 'bg-emerald-500'}`}></div>
                  <span className="text-xs text-gray-600">{isError ? 'Sync Offline' : 'Global Sync Active'}</span>
                </div>
              </div>
            </CardContent>
          </Card>
