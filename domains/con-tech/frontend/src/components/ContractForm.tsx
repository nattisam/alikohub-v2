...
export const AddChangeOrders: React.FC<{ contractid: number }> = () => {
  // const uploadContractMutation = useUploadContract(); // This is commented out, so it's effectively unused.
  // const updateContractStatusMutation = useUpdateContractStatus(); // This is commented out, so it's effectively unused.
  ...
  const handleAddChangeOrder = async () => {
    try {
      ...
      setChangingOrder({
        ...changingOrder,
        error: "Error while adding change orders! Try again later.",
      });
    } finally {
