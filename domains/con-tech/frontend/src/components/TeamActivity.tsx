...
const TeamActivity: React.FC<TeamActivityProps> = ({ className = '' }) => {
  ...
  const fetchActivities = async () => {
    ...
    try {
      ...
      // Fetch recent tasks
      const tasks = await tasksService.findByProject(selectedProject, { take: 3 });
      
      // Fetch recent inspections
      const inspections = await inspectionsService.findAllForProject(selectedProject, { take: 3 });
      ...
      const inspectionActivities: Activity[] = inspections.map(inspection => ({
        user: 'Inspector',
      ...
      setActivities([
        { user: 'John', action: 'completed Foundation Inspection', time: '2 hours ago' },
        { user: 'Sarah', action: 'uploaded safety protocol update', time: '4 hours ago' },
        ...
        <div className="space-y-4">
          {displayActivities.map((activity, index) => (
            <div key={index} className="flex items-center space-x-2">
              ...
              <p>
                {activity.user} {activity.action} <span className="text-gray-500">{activity.time}</span>
              </p>
            </div>
          ))}
        </div>
