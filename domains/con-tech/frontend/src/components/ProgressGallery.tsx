...
const ProgressGallery: React.FC<ProgressGalleryProps> = ({
  className = "",
}) => {
    ...
    try {
      ...
      for (const project of projects.slice(0, 3)) {
        try {
          ...
          console.error(
            `Error fetching inspections for project ${project.id}:`,
            err,
          );
        }
      }
      ...
      if (allImages.length === 0) {
        setImages([
          {
            url: "https://placehold.co/600x400/cccccc/969696?text=Progress+1",
            caption: "Site Preparation",
          },
          {
            url: "https://placehold.co/600x400/cccccc/969696?text=Progress+2",
            caption: "Foundation Work",
          },
          {
            url: "https://placehold.co/600x400/cccccc/969696?text=Progress+3",
      ...
      <section className={`bg-white p-4 rounded-lg shadow-md ${className}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[1, 2, 3].map((index) => (
            <div
              key={index}
              className="w-full h-32 bg-gray-200 rounded animate-pulse"
            ></div>
          ))}
        </div>
      </section>
