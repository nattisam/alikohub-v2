...
export class TasksController {
    ...
    async create(@Payload() payload: { dto: CreateTaskDto; user: AuthenticatedUser }) {
        this.logger.log(`Creating task for project ID: ${payload.dto.projectId} by user: ${payload.user.firebaseId}`);
        try {
            return await this.tasksService.create(payload.dto, payload.user);
        } catch (error) {
            this.logger.error(`Failed to create task for project ID ${payload.dto.projectId} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findByProject(@Payload() payload: { projectId: number; query: any; user: AuthenticatedUser }) {
        this.logger.log(`Fetching tasks for project ID: ${payload.projectId} (requested by: ${payload.user.firebaseId})`);
        try {
            return await this.tasksService.findByProject(payload.projectId, payload.query, payload.user);
        } catch (error) {
            this.logger.error(`Failed to fetch tasks for project ID ${payload.projectId} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async findOne(@Payload() payload: { id: number; user: AuthenticatedUser }) {
        this.logger.log(`Fetching task details for ID: ${payload.id} by user: ${payload.user.firebaseId}`);
        try {
            return await this.tasksService.findOne(payload.id, payload.user);
        } catch (error) {
            this.logger.error(`Failed to fetch task details for ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async update(@Payload() payload: { id: number; dto: UpdateTaskDto; user: AuthenticatedUser }) {
        this.logger.log(`Updating task ID: ${payload.id} by user: ${payload.user.firebaseId}`);
        try {
            return await this.tasksService.update(payload.id, payload.dto, payload.user);
        } catch (error) {
            this.logger.error(`Failed to update task ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async remove(@Payload() payload: { id: number; user: AuthenticatedUser }) {
        this.logger.log(`Removing task ID: ${payload.id} by admin: ${payload.user.firebaseId}`);
        try {
            return await this.tasksService.remove(payload.id, payload.user);
        } catch (error) {
            this.logger.error(`Failed to remove task ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async updateProgress(@Payload() payload: { id: number; progress: number; user: AuthenticatedUser }) {
        this.logger.log(`Updating progress for task ID: ${payload.id} to ${payload.progress}% by: ${payload.user.firebaseId}`);
        try {
            return await this.tasksService.updateTaskProgress(payload.id, payload.progress, payload.user);
        } catch (error) {
            this.logger.error(`Failed to update progress for task ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
    ...
    async getStats(@Payload() payload: { projectId?: number; assignedTo?: string; user: AuthenticatedUser }) {
        this.logger.log(`Fetching task stats for project: ${payload.projectId || 'all'}, assignedTo: ${payload.assignedTo || 'all'} (requested by: ${payload.user.firebaseId})`);
        try {
            return await this.tasksService.getTaskStats(payload.user, payload.projectId, payload.assignedTo);
        } catch (error) {
            this.logger.error(`Failed to fetch task stats for user ${payload.user.firebaseId}: ${error.message}`, error.stack);
            throw error;
        }
    }
}