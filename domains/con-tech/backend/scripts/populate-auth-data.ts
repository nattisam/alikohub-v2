...
const CONTECH_DB_URL = 'postgresql://contech_user:contech_pass@localhost:5433/contechdb';
...
async function populate() {
    ...
    try {
        ...
        for (const user of users) {
             ...
             const resultContech = await contechClient.query(contechUpsertQuery, [user.firebaseId]);
             if (resultContech.rowCount && resultContech.rowCount > 0) {
                 createdContechProfiles++;
             }

             // 3. Upsert into Auth Service DB ("ContechUser")
             const checkAuth = await authClient.query('SELECT 1 FROM "ContechUser" WHERE "userId" = $1', [user.firebaseId]);
        ...
        console.log(`ContechProfiles check/create (Con-Tech DB): ${createdContechProfiles}`);
