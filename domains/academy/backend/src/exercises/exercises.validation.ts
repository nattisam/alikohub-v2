...
export const CreateExerciseSchema = Joi.object({
  dto: Joi.object({
    ...
    description: Joi.string().allow('', null).optional().description('Detailed instructions for the exercise'),
    moduleId: Joi.number().integer().required().description('The module ID this exercise belongs to'),
    lessonId: Joi.number().integer().optional().description('The lesson ID this exercise belongs to'),
    type: Joi.string().valid('MULTIPLE_CHOICE', 'TRUE_FALSE', 'MATCHING', 'SHORT_TEXT').required().description('The type of exercise'),
    question: Joi.string().required().description('The question text'),
    options: Joi.any().optional().description('Options for multiple choice etc.'),
    correctAnswer: Joi.any().optional().description('The correct answer'),
    hints: Joi.array().items(Joi.string()).optional().description('Hints for the student'),
    points: Joi.number().integer().optional().default(1).description('Points for this exercise'),
    order: Joi.number().integer().optional().default(0).description('Order of the exercise')
...
export const UpdateExerciseSchema = Joi.object({
  ...
  dto: Joi.object({
    ...
    description: Joi.string().allow('', null).optional().description('The updated instructions'),
    ...
    type: Joi.string().valid('MULTIPLE_CHOICE', 'TRUE_FALSE', 'MATCHING', 'SHORT_TEXT').optional().description('The updated type'),
    question: Joi.string().optional().description('The updated question'),
    options: Joi.any().optional().description('The updated options'),
    correctAnswer: Joi.any().optional().description('The updated correct answer'),
    hints: Joi.array().items(Joi.string()).optional().description('The updated hints'),
