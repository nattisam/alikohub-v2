...
export class LessonsController {
  ...
  async create(@Payload() payload: { dto: CreateLessonDto; user: AuthenticatedUser }) {
    this.logger.log(`Creating lesson "${payload.dto.title}" in module ${payload.dto.moduleId} by user: ${payload.user.firebaseId}`);
    try {
      return await this.lessonsService.create(payload.dto, payload.user);
    } catch (error) {
      this.logger.error(`Failed to create lesson "${payload.dto.title}" in module ${payload.dto.moduleId} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findByModule(@Payload() payload: { moduleId: number; user: AuthenticatedUser; query?: any }) {
    this.logger.log(`Fetching lessons for module ID: ${payload.moduleId} by user: ${payload.user.firebaseId}`);
    try {
      return await this.lessonsService.findByModule(payload.moduleId, payload.user, payload.query);
    } catch (error) {
      this.logger.error(`Failed to fetch lessons for module ID ${payload.moduleId} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findInstructorLessons(@Payload() payload: { user: AuthenticatedUser; query?: any }) {
    this.logger.log(`Instructor ${payload.user.firebaseId} fetching their lessons`);
    try {
      return await this.lessonsService.findByInstructor(payload.user, payload.query);
    } catch (error) {
      this.logger.error(`Instructor ${payload.user.firebaseId} failed to fetch their lessons: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findOne(@Payload() payload: { id: number; user: AuthenticatedUser }) {
    this.logger.log(`Fetching details for lesson ID: ${payload.id} by user: ${payload.user.firebaseId}`);
    try {
      return await this.lessonsService.findOne(payload.id, payload.user);
    } catch (error) {
      this.logger.error(`Failed to fetch details for lesson ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async update(@Payload() payload: { id: number; dto: UpdateLessonDto; user: AuthenticatedUser }) {
    this.logger.log(`Updating lesson ID: ${payload.id} by user: ${payload.user.firebaseId}`);
    try {
      return await this.lessonsService.update(payload.id, payload.dto, payload.user);
    } catch (error) {
      this.logger.error(`Failed to update lesson ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async remove(@Payload() payload: { id: number; user: AuthenticatedUser }) {
    this.logger.log(`Removing lesson ID: ${payload.id} by user: ${payload.user.firebaseId}`);
    try {
      return await this.lessonsService.remove(payload.id, payload.user);
    } catch (error) {
      this.logger.error(`Failed to remove lesson ID ${payload.id} by user ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
}