...
export class CohortAccessGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (cohortId) {
            return await this.checkCohortAccess(user.id, parseInt(cohortId), user.role);
        }
    ...
    private async checkCohortAccess(userId: number|string, cohortId: number, userRole: string): Promise<boolean> {
    ...
    private async checkCourseAccess(userId: number|string, courseId: number, userRole: string): Promise<boolean> {
        // Convert userId to string (Prisma expects string for userId)
        const userIdStr = userId.toString();
