...
export const courseApi = {
  ...
  // Get all courses for admin - no status filter
  getAllCoursesForAdmin: async (params?: Record<string, unknown>) => {
    try {
      const response = await academyApi.get("/academy/courses/all", { params });
      return response;
    } catch (error) {
      console.error("API Error:", error);
      throw error;
  ...
  getPendingCourses: async (params?: Record<string, unknown>) => {
    try {
      const response = await academyApi.get("/academy/courses/pending", {
        params,
      });
      return response;
    } catch (error) {
  ...
  updateCourse: (id: number, data: unknown) =>
    academyApi.patch(`academy/courses/${id}`, data),
  ...
  updateModule: (id: number, data: unknown) =>
    academyApi.put(`/academy/modules/${id}`, data),
  deleteModule: (id: number) => academyApi.delete(`/academy/modules/${id}`),

  // Cohorts
  getCohorts: (courseId: number) =>
    academyApi.get(`/academy/cohorts`, { params: { courseId } }),
  getCohort: (id: number) => academyApi.get(`/academy/cohorts/${id}`),
  createCohort: (data: unknown) => academyApi.post("/academy/cohorts", data),
  updateCohort: (id: number, data: unknown) =>
    academyApi.patch(`/academy/cohorts/${id}`, data),
  ...
  updateLesson: (id: number, data: unknown) =>
    academyApi.put(`/academy/lessons/${id}`, data),
  ...
  submitForApproval: (id: number) =>
    academyApi.post(`/academy/courses/${id}/submit`),
  approveCourse: (id: number) =>
    academyApi.post(`/academy/courses/${id}/approve`),
