...
import type { LessonContent } from "../common/types.d";
...
const ContentViewer: React.FC<ContentViewerProps> = ({
  ...
  const fetchContent = async () => {
    try {
      setLoading(true);
      const response = await courseApi.getContent(lessonId);
  ...
  const updateContentProgress = async (
    ...
    try {
      ...
      console.log(
        `Progress updated for content ${contentId} with status ${status}`,
      );
    } catch (error) {
  ...
  const updateLessonProgress = async (progress: number) => {
    if (!moduleId || !currentUser) {
      console.log(
        "Missing moduleId or currentUser, skipping lesson progress update",
      );
      return;
    }
        ...
        if (isYouTubeUrl(content.url)) {
          ...
          return (
            <div className="w-full">
              ...
              <div className="mt-4 p-4 bg-blue-50 rounded-lg">
                <p className="text-sm text-blue-800">
                  <strong>Progress:</strong> Content marked as viewed when
                  played
                </p>
              </div>
            </div>
  ...
  useEffect(() => {
    if (contents.length > 0 && activeContentIndex < contents.length) {
      ...
      const progress = Math.min(
        100,
        Math.round(((activeContentIndex + 1) / contents.length) * 100),
      );
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full mx-4">
          ...
          <p className="mb-6 text-gray-600 text-center">
            There is no content for this lesson yet.
          </p>
          <button
            onClick={onClose}
            className="w-full py-3 bg-gradient-to-r from-gray-600 to-gray-700 text-white rounded-xl hover:from-gray-700 hover:to-gray-800 font-medium transition-all shadow-md hover:shadow-lg"
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 pb-4 border-b border-gray-200">
            <div>
              ...
              <p className="text-gray-600 mt-1">
                Review all content items for this lesson
              </p>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            <div className="lg:w-1/3">
              <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center">
                <FaBook className="mr-2 text-blue-600" /> Content Items (
                {contents.length})
              </h3>
