...
const AuthPromptModal: React.FC<AuthPromptModalProps> = ({
  ...
  const buildRedirectQuery = () => {
    const redirectUrl = courseId
      ? encodeURIComponent(`/courses/${courseId}`)
      : encodeURIComponent(location.pathname + location.search);
    return `?redirect=${redirectUrl}&message=Sign up or log in to unlock full course content.`;
  };
  ...
  const handleSignup = () => {
    navigate(`/auth/signup${buildRedirectQuery()}`);
    onClose();
  };
