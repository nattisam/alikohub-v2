    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6 text-center">
        <ErrorState
          ...
          message={error || "Could not find the requested lesson."}
          onRetry={() => navigate(-1)}
        />
      </div>
    );
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden sticky top-24">
              ...
              <div className="divide-y divide-gray-50 max-h-[calc(100vh-160px)] overflow-y-auto">
                {moduleLessons.map((l) => (
                  ...
                  >
                    <div
                      className={`flex-shrink-0 p-1.5 rounded-md ${l.id === lesson.id ? "bg-[#3E92D1] text-white" : "bg-gray-50 text-gray-400"}`}
                    >
                      {l.id === lesson.id ? (
                        <PlayCircle size={14} />
                      ) : l.contents && l.contents.length > 0 ? (
                        getContentIcon(l.contents[0].type)
                      ) : (
                        getContentIcon(l.type)
                      )}
                    </div>
                    <span
                      className={`text-xs font-semibold truncate ${l.id === lesson.id ? "text-[#3E92D1]" : "text-gray-700"}`}
                    >
                      {l.title}
                    </span>
                  </button>
