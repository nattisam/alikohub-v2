    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
         <div className="text-center">
            <div className="text-red-500 mb-2">Failed to load course content</div>
            <Link to="/student-dashboard/mycourses" className="text-blue-600 hover:underline">Return to My Courses</Link>
         </div>
      </div>
    );
  }
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
              ...
              <div className="flex flex-wrap gap-6 text-sm text-gray-500 pt-4 border-t border-gray-100">
                ...
                <span className="flex items-center gap-2">
                   ...
                   {courseData?.estimatedTime ? `${Math.floor(courseData.estimatedTime / 60)}h ${courseData.estimatedTime % 60}m` : 'N/A'}
                </span>
                <span className="flex items-center gap-2">
                  ...
                  <div className="flex items-start justify-between mb-4">
                     <div>
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500 mt-1">
                           <Clock size={14} />
                           {selectedLesson?.duration ? `${selectedLesson.duration}m` : '15 min'}
                        </div>
                     </div>
                   ...
                   {modules.map((module, idx) => (
                      <div key={module.id} className="border border-gray-100 rounded-md overflow-hidden">
                         ...
                         >
                            ...
                            {expandedModules[module.id] ? <ChevronDown size={14} className="text-gray-500" /> : <ChevronRight size={14} className="text-gray-500" />}
                         </button>
                               ...
                               {module.lessons
                                  ...
                                  >
                                     ...
                                     <div className="flex-1 min-w-0">
                                        <p className={`font-medium truncate ${selectedLesson?.id === lesson.id ? 'text-[#3E92D1]' : 'text-gray-700'}`}>
                                           {lesson.title}
                                        </p>
                                        <p className="text-xs text-gray-400 mt-0.5">
                                           {lesson.duration ? `${lesson.duration}m` : '15m'}
                                        </p>
                                     </div>
