...
export function ProgramsSnapshot() {
  ...
  // Use real data from API, map fields for the component
  const displayPrograms = dbPrograms.map((dbp: any) => ({
    ...dbp,
    name: dbp.title,
    image: dbp.thumbnail,
    credential: dbp.category || "Healthcare Training",
    duration: dbp.duration || "Self-paced",
    modality: dbp.modality || "Online",
    location: dbp.location || "Remote",
    enrollmentStatus: dbp.enrollmentStatus || "open",
    startDate: dbp.startDate || "Immediate",
  }));
        ...
        {isLoading ? (
          ...
          <div className="text-center py-20 bg-muted/10 rounded-2xl border border-dashed">
            <p className="text-muted-foreground">
              No Health programs currently available.
            </p>
          </div>
        )}
