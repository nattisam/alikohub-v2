...
export class AuthService {
  ...
  async login(dto: any) {
    ...
    // Ensure domain-specific user records exist (sync with all services)
    try {
      await this.syncContechUser(user.firebaseId);
      await this.syncEventsUser(user.firebaseId);
      await this.syncAcademyUser(user.firebaseId);
    } catch (syncError) {
      this.logger.warn(`Failed to sync domain user records during login:`, syncError);
      // Don't fail the login if sync fails, just log the warning
    }

    // Re-fetch user to get updated relations after sync
    const updatedUser = await this.userService.findByFirebaseId(user.firebaseId);
    ...
    this.logger.log(`Login successful for user: ${updatedUser.id}`);
  ...
  private toPlain(user: any) {
    ...
    if (user.academyUser) {
      ...
      // Ensure default values if no academyUser exists
      plain.academyRole = null;
      plain.academyActiveRole = null;
      plain.academyStatus = null;
    }
    ...
    if (user.contechUser) {
      ...
      // Ensure default values if no contechUser exists
      plain.contechRole = null;
      plain.contechStatus = null;
    }
    ...
    if (user.eventsUser) {
      ...
      // Ensure default values if no eventsUser exists
      plain.eventsRole = null;
      plain.eventsStatus = null;
    }
