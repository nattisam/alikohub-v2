...
export class EventsController {
  ...
  async createPost(
    ...
    return this.eventsClient.send(
      { cmd: "create_post" },
      { dto: createPostDto, user: req.user },
    );
  }

  @ApiOperation({
    summary: "Get posts for management (supports filtering by status/type)",
  })
  @Get()
  ...
  async updatePost(
    ...
    return this.eventsClient.send(
      { cmd: "update_post" },
      { id, dto: updatePostDto, user: req.user },
    );
  }
  ...
  @ApiOperation({
    summary: "Get a specific post for management (Author or Admin only)",
  })
  @Get(":id")
  findPostById(@Request() req: RequestWithUser, @Param("id") id: string) {
    return this.eventsClient.send(
      { cmd: "find_post_by_id" },
      { id, user: req.user, public: false },
    );
  }
  ...
  findAllPromotionRequests(@Request() req: RequestWithUser) {
    return this.eventsClient.send(
      { cmd: "find_all_promotion_requests" },
      { user: req.user },
    );
  }

  @ApiOperation({
    summary: "Mark a promotion request as reviewed (Admin only)",
  })
  ...
  markPromotionReviewed(
    ...
    return this.eventsClient.send(
      { cmd: "mark_promotion_request_reviewed" },
      { id, user: req.user },
    );
  }
}
