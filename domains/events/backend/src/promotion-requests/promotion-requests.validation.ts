...
export const CreatePromotionRequestSchema = Joi.object({
  ...
  type: Joi.string().valid('EVENT', 'ANNOUNCEMENT', 'NEWS').required().description('Type of content to promote'),
  message: Joi.string().required().description('Brief description of the promotion or company')
