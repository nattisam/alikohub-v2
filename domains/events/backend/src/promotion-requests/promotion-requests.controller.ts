...
export class PromotionRequestsController {
  ...
  async create(@Payload() dto: CreatePromotionRequestDto) {
    this.logger.log(`Submitting new promotion request from company: ${dto.companyName}`);
    try {
      return await this.service.create(dto);
    } catch (error) {
      this.logger.error(`Failed to submit promotion request for ${dto.companyName}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async findAll(@Payload() payload: { user: AuthenticatedUser }) {
    this.logger.log(`Fetching all promotion requests by admin: ${payload.user.firebaseId}`);
    try {
      return await this.service.findAll(payload.user);
    } catch (error) {
      this.logger.error(`Failed to fetch promotion requests for admin ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
  ...
  async markAsReviewed(@Payload() payload: { id: string; user: AuthenticatedUser }) {
    this.logger.log(`Marking promotion request ID: ${payload.id} as reviewed by admin: ${payload.user.firebaseId}`);
    try {
      return await this.service.markAsReviewed(payload.id, payload.user);
    } catch (error) {
      this.logger.error(`Failed to update promotion request ID ${payload.id} by admin ${payload.user.firebaseId}: ${error.message}`, error.stack);
      throw error;
    }
  }
}
